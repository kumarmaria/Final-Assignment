...
def get_scatter_chart(entered_site, selected_payload):
    minp, maxp = selected_payload
    filtered_df = spacex_df[(spacex_df['Payload Mass (kg)']>minp) & (spacex_df['Payload Mass (kg)']<maxp)]
    if entered_site == 'ALL':
    ...
    filtered_df=spacex_df[filtered_df['Launch Site']== entered_site]
        fig = px.scatter(filtered_df, 
        x='Payload Mass (kg)', 
        y='class',
        color='Booster Version Category',
